...
st.write("Lokasi Analisis: **Desa Lampeuneurut Gampong, Kec. Darul Imarah, Aceh Besar**")
...
koordinat_pusat = [5.5158, 95.3122] # Koordinat Desa Lampeuneurut
...
with kolom_peta:
    st.subheader("Peta Lokasi Fasilitas Desa")
    st_folium(peta, width="100%", height=500)

with kolom_data:
    st.subheader("Tabel Koordinat Objek UAS")
    st.dataframe(df_filtered, use_container_width=True)
    st.write(f"Menampilkan **{len(df_filtered)}** objek di Desa Lampeuneurut.")
